...
<svelte:head>
  ...
  <meta name="description" content="Ubah dimensi lebar dan tinggi gambar Anda secara instan. Menjaga rasio aspek dengan mudah. 100% aman dan berjalan di browser." />
</svelte:head>
...
>
    ...
    <div class="space-y-6">
      ...
      {:else}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-2">
            <Text tag="span" variant="muted" class="text-xs uppercase font-bold text-gray-500">Asli: {originalWidth}x{originalHeight}</Text>
            <ImagePreview src={previewSrc} />
          </div>
          <div class="space-y-2">
            <Text tag="span" variant="muted" class="text-xs uppercase font-bold text-blue-600">Hasil: {width}x{height}</Text>
    ...
    >
      ...
      <div class="flex items-center space-x-2">
        <input 
          ...
          class="h-4 w-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500"
          disabled={!selectedFile || processing}
        />
        <label for="maintain-ratio" class="text-sm font-medium text-gray-700 cursor-pointer">Jaga Rasio Aspek</label>
      </div>
  ...
  {#snippet seoText()}
    ...
    <p>Gunakan pengukur dimensi gambar kami untuk mengubah resolusi foto Anda tanpa perlu mengunduh aplikasi khusus. Berikut keunggulannya:</p>
    <ul>
      <li><strong>Akurat:</strong> Tentukan lebar (width) dan tinggi (height) secara presisi dengan pixel.</li>
      <li><strong>Rasio Aspek:</strong> Jaga proporsi gambar asli agar tidak terlihat gepeng atau melar.</li>
      <li><strong>Canvas API:</strong> Proses perubahan ukuran dilakukan dengan algoritma perataan browser untuk hasil terbaik.</li>
    </ul>
  {/snippet}
</ToolPageTemplate>
