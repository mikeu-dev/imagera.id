...
<svelte:head>
	...
	<meta
		name="description"
		content="Kurangi ukuran file gambar JPG, PNG, atau WEBP Anda tanpa kehilangan kualitas secara signifikan. 100% aman dan berjalan di browser."
	/>
</svelte:head>
...
>
		...
		<div class="space-y-6">
			...
			{:else}
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div class="space-y-2">
						<Text tag="span" variant="muted" class="text-xs font-bold uppercase">Asli</Text>
	...
	{#snippet controls()}
		...
		>
			...
			<Text tag="p" variant="muted" class="text-xs italic">
				Kualitas lebih rendah menghasilkan ukuran file yang lebih kecil tetapi detail gambar
				berkurang.
			</Text>
		</ToolControls>
	{/snippet}
	...
	{#snippet seoText()}
		...
		<p>
			Alat kompresi gambar kami menggunakan teknologi Canvas API browser modern untuk memproses file
			Anda secara lokal. Ini berarti gambar Anda tidak pernah diunggah ke server manapun, menjamin
			privasi data Anda 100%.
		</p>
		<ul>
			<li><strong>Privasi Penuh:</strong> Data Anda tetap di perangkat Anda.</li>
			<li>
				<strong>Kecepatan Instan:</strong> Tidak ada waktu tunggu upload/download dari server.
			</li>
			<li>
				<strong>Pratinjau Langsung:</strong> Lihat perbedaan kualitas sebelum mengunduh hasil.
			</li>
		</ul>
	{/snippet}
</ToolPageTemplate>
