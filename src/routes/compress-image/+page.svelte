<script lang="ts">
	...
	// For legacy single file preview (last processed or selected)
	...
	function handleUpload(files: File[]) {
		...
		// Auto select first file as active if none
	...
	async function processItem(item: QueueItem) {
		...
		try {
			const blob = await compressImage(item.file, quality / 100);
			item.resultBlob = blob;
			item.status = 'done';
			
			// Update active preview if it's this item
	...
	async function handleProcess() {
		...
		try {
			...
			toast.success(m.status_done() + ' ' + m.nav_compress());
		} finally {
			processing = false;
		}
	}
	...
	async function handleDownloadZip() {
		...
		completed.forEach(item => {
			const fileName = `compressed-${item.file.name.replace(/\.[^/.]+$/, '')}.jpg`;
		...
		link.download = `imagera-batch-compressed.zip`;
		link.click();
	}
	...
	function handleDownload() {
		if (queue.length > 1) {
			...
			link.download = `compressed-${activeOriginalFile.name.replace(/\.[^/.]+$/, '')}.jpg`;
			link.click();
		}
	}
						...
						<div class="grid grid-cols-1 gap-4">
							<div class="space-y-2">
								<Text tag="span" variant="muted" class="text-xs font-bold text-blue-600 uppercase"
									>{m.tool_result()}</Text
								>
								<div class="relative overflow-hidden rounded-3xl border border-gray-100 bg-gray-50 shadow-inner">
									<ImagePreview src={activeResultSrc} loading={processing} />
									{#if !activeResultSrc && queue.length > 0}
										<div class="flex h-[300px] items-center justify-center p-12 text-center text-sm font-bold text-gray-400">
											{m.status_waiting()}...
										</div>
									{/if}
								</div>
